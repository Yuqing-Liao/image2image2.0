...
from Process_web  import send_email, get_md5, save_user_avatar
from Process_web import get_current_site
...
def authorize(request):
    ...
    if user:
        if not user.nikename or not user.nikename.strip():
            import datetime
            user.autoname = "" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
        try:
